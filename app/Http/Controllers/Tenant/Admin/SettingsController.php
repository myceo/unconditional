...
class SettingsController extends Controller
{
 ...
 /*   public function saveSettings(Request $request){
        ...
        if(!empty($files)){
            foreach($files as $key=>$value){
                if($request->hasFile($key)){
                    ...
                    //@unlink($setting->value);
                    if(!empty($setting->value) && Storage::has($setting->value)) {
                        Storage::delete($setting->value);
                    }

                    $tempFile = $request->file($key)->path();
                    ...
                    $path = Storage::putFile(SETTINGS,$tempFile);
    ...
    public function removePicture(Setting $setting){
        ...
        //@unlink($setting->value);
        if(!empty($setting->value) && Storage::has($setting->value))  Storage::delete($setting->value);
        $setting->value = null;
        $setting->save();
    ...
    public function language(){
        ...
        return view('admin.settings.language',compact('languages','controller','timezones'));
    ...
    public function saveLanguage(Request $request){
        $this->validate($request,[
            ...
            'config_timezone'=>'required'
        ]);

        Setting::where('key','config_language')->update(['value'=>$request->config_language]);
        Setting::where('key','config_timezone')->update(['value'=>$request->config_timezone]);
        return back()->with('flash_message',__('admin.changes-saved'));
    }
