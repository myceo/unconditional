...
function getDepartmentAdmins(){
    return getDepartment()->users()->wherePivot('department_admin', 1)->get();
}

function getAge($birth){
    $birth = \Illuminate\Support\Carbon::parse($birth)->timestamp;
    $t = time();
...
function getNewFilePath($filePath){
    $newFileName = \Illuminate\Support\Str::random();
...
function copyCourseFile($filePath){
    $newPath = getNewFilePath($filePath);
    Storage::copy($filePath, $newPath);
    return $newPath;
}

function canEnroll(Course $course){
    if(empty($course->enabled)){

        return false;
    }

    if(!empty($course->end_date) && \Illuminate\Support\Carbon::parse($course->end_date) < now()){

        return false;
    }

    if(!empty($course->closes_on) && \Illuminate\Support\Carbon::parse($course->closes_on) < now()){
        return false;
    }
...
function lessonsCompleted(User $user, Course $course){
...
function testsCompleted(User $user, Course $course){
    ...
    foreach ($course->tests as $test){
        if(!$user->userTests()->where('test_id','=',$test->id)->where('score','>=',$test->passmark)->first()){
...
function totalUserCourseTests(User $user, Course $course){
    $count = 0;
    foreach ($course->tests as $test){
        if($user->userTests()->where('test_id','=',$test->id)->where('score','>=',$test->passmark)->first()){
            $count++;
...
function courseCompleted(User $user, Course $course){
    return lessonsCompleted($user,$course) && testsCompleted($user,$course);
}
...
function dateFieldString($date,$format='Y-m-d'){
    if(empty($date)){
        return '';
    }
...
function userLink(User $user){
  return   url('/admin/members/' . $user->id);
  //  return route('members.show',['member'=>$user->id]);
}
