...
@section('pageTitle',$announcement->title)
...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            ...
            <h4>{{ $announcement->title }}
                ...
                @endif
                  ({{ \Carbon\Carbon::parse($announcement->created_at)->format('D d/M/Y') }})
            </h4>

            @if($announcement->enable_comments==1)
    ...
    @foreach($announcementComments as $thread)
                ...
                <div class="tickets">
                    <div class="ticket-content">
                        ...
                        <div class="ticket-description thread-text">
                            ...
                            @if($thread->announcementCommentAttachments()->count()>0)
                                <div class="ticket-divider"></div>
                                <p class="m-b-md">
                                    <span>
                                        <a  data-toggle="tooltip" title="@lang('admin.download-all')"  href="{{ route('member.announcement-comments.download-attachments',['announcementComment'=>$thread->id]) }}">
                                        <i class="fa fa-paperclip"></i> {{ $thread->announcementCommentAttachments()->count() }} @if($thread->announcementCommentAttachments()->count()>1) @lang('admin.attachments') @else @lang('admin.attachment') @endif
                                        </a>
                                    </span>
                                ...
                                <div class="gallery">
                                    ...
                                    @foreach($thread->announcementCommentAttachments as $attachment)
                                        <div onclick="document.location.replace('{{ route('member.announcement-comments.download-attachment',['announcementCommentAttachment'=>$attachment->id]) }}')" class="gallery-item"  data-toggle="tooltip" title="{{ $attachment->file_name }}" data-original-title="{{ basename($attachment->file_path) }}"
                                             @if(isImage($attachment->file_path))
                                             data-image="{{ route('member.announcement-comments.view-image',['announcementCommentAttachment'=>$attachment->id]) }}"
                                             style="background-image: url('{{ route('member.announcement-comments.view-image',['announcementCommentAttachment'=>$attachment->id]) }}');"
                                             @else
    ...
    @if ($announcementComments->hasPages())
        <div class="card">
            <div class="card-body">
                <div class="row">
                    <div class="col-md-12">
                        {{ $announcementComments->links() }}
                    </div>
                </div>
    ...
    @if($announcement->enable_comments==1)
        <div class="card" id="replybox">
            ...
            <div class="card-body">
                ...
                <form method="POST" action="{{ route('member.announcement-comments.store',['announcement'=>$announcement->id]) }}" accept-charset="UTF-8" class="form-horizontal" enctype="multipart/form-data">
                    {{ csrf_field() }}
                    <input type="hidden" name="msg_id" value="{{ $msgId }}"/>
                    @include ('member.announcement-comments.form', ['formMode' => 'edit'])
