...
@section('breadcrumb')
    ...
    <li><span>@lang('admin.sent-messages')</span>
    </li>
@endsection
    ...
    <div class="card">
        <div class="card-header">
            ...
            <div class="card-header-form">
                <form  method="GET" action="{{ url('/member/emails') }}" >
        ...
        <form id="msg-list" action="{{ route('member.email.delete-multiple') }}" onsubmit="return confirm('@lang('admin.delete-prompt')')" method="post">
            @csrf
                        ...
                        @foreach($emails as $item)
                            <tr>
                                ...
                                <td  class="clickable truncate">
                                    ...
                                    <a href="{{ url('/member/emails/' . $item->id) }}" class="wide-link"  >
                                ...
                                <td  onclick="document.location.replace('{{ url('/member/emails/' . $item->id) }}')"  class="clickable">
                                    <a  class="wide-link" href="{{ url('/member/emails/' . $item->id) }}">{{ $item->subject }}</a>
                                </td>
                                <td  onclick="document.location.replace('{{ url('/member/emails/' . $item->id) }}')"  class="clickable">
                                    <a  style="width: 100%; height: 100%" href="{{ url('/member/emails/' . $item->id) }}">
                                        @if($item->emailAttachments()->count()>0)
                                            <i class="fas fa-paperclip"></i>
                                        @endif
                                    </a>
                                </td>
                                <td  onclick="document.location.replace('{{ url('/member/emails/' . $item->id) }}')"  class="clickable">{{ \Illuminate\Support\Carbon::parse($item->crated_at)->format('D, M d, Y') }}</td>
                                <td>
                                    <a  href="{{ route('member.email.delete',['id'=>$item->id]) }}"  title="@lang('site.delete')" onclick="return confirm('@lang('admin.delete-prompt')')" ><i class="fa fa-trash"></i></a>
                                </td>
                            </tr>
                        @endforeach
                        </tbody></table>
