...
@section('pageTitle',__('site.test').': '.$test->name)
@section('innerTitle',__('site.test').': '.$test->name)
@section('breadcrumb')
    <li  class="breadcrumb-item"><a href="{{ route('site.tests') }}">@lang('site.tests')</a></li>
...
@section('content')
    <div class="row_">
        ...
        <div  >
            ...
            <div >
                <form class="int_hpw" id="testform" method="post" action="{{ route('site.tests.process',['userTest'=>$userTest->id,'course'=>isset($course)?$course:null])  }}">
                    @csrf
                    <div >

                        <?php $count = 0; ?>
                        @foreach($test->testQuestions()->ordered()->get() as $question)
                            <?php $count++; ?>
                            <div class="card question" id="question{{ $count }}">
                                <div class="card-header">
                                    <h3 class="panel-title">{{ $count }}. {!! clean( ($question->question) ) !!}</h3>
                                </div>
                                <div class="card-body">
                                    <p >
