...
@section('innerTitle','test'.' #'.$test->id)
            ...
            <div class="col-md-12">
                <div  >
                    <div  >
                        ...
                        <button type="submit" form="deleteform" class="btn btn-danger btn-sm" title="@lang('site.delete')" onclick="return confirm(&quot;@lang('site.confirm-delete')&quot;)"><i class="fa fa-trash-o" aria-hidden="true"></i> @lang('site.delete')</button>
                        <form id="deleteform" method="POST" action="{{ url('admin/tests' . '/' . $test->id) }}" accept-charset="UTF-8" class="int_inlinedisp">
                            {{ method_field('DELETE') }}
                            {{ csrf_field() }}
