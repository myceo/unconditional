...
<div class="form-group {{ $errors->has('status') ? 'has-error' : ''}}">
    ...
    @foreach (json_decode('{"0":"'.__('site.disabled').'","1":"'.__('site.enabled').'"}', true) as $optionKey => $optionValue)
...
<div class="form-group {{ $errors->has('private') ? 'has-error' : ''}}">
    <label for="private" class="control-label">@lang('site.visibility')</label>
    <select name="private" class="form-control" id="private" >
        @foreach (json_decode('{"0":"'.__('site.public').'","1":"'.__('site.private').'"}', true) as $optionKey => $optionValue)
            <option value="{{ $optionKey }}" {{ ((null !== old('private',@$test->private)) && old('private',@$test->private) == $optionKey) ? 'selected' : ''}}>{{ $optionValue }}</option>
        @endforeach
    </select>
    <p class="help-block">@lang('site.private-test-help')</p>
    {!! clean( $errors->first('private', '<p class="help-block">:message</p>') ) !!}
</div>
