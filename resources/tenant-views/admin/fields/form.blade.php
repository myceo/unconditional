...
<div class="form-group {{ $errors->has('public') ? 'has-error' : ''}}">
    <label for="public" class="control-label">@lang('admin.public')</label>
    <select name="public" class="form-control" id="public" >
        @foreach (json_decode('{"1":"'.__('admin.yes').'","0":"'.__('admin.no').'"}', true) as $optionKey => $optionValue)
            <option value="{{ $optionKey }}" {{ (isset($field->public) && $field->public == $optionKey) ? 'selected' : ''}}>{{ $optionValue }}</option>
        @endforeach
    </select>
