...
@section('content')
    ...
    <div class="card">
        ...
        <form id="msg-list" action="{{ route('email.delete-multiple-inbox') }}" onsubmit="return confirm('@lang('admin.delete-prompt')')" method="post">
            @csrf
                ...
                <table class="table">
                    ...
                    <tr @if($item->pivot->read==0) class="unread" @endif >
                        ...
                        <td  onclick="document.location.replace('{{ route('email.view-inbox',['email'=>$item->id]) }}')"  class="clickable">
                            <a  class="wide-link" href="{{ route('email.view-inbox',['email'=>$item->id]) }}">{{ $item->subject }}</a>
                        </td>
                        <td  onclick="document.location.replace('{{ route('email.view-inbox',['email'=>$item->id]) }}')"  class="clickable">
                        ...
                        <td  onclick="document.location.replace('{{ route('email.view-inbox',['email'=>$item->id]) }}')"  class="clickable">{{ \Illuminate\Support\Carbon::parse($item->crated_at)->format('D, M d, Y') }}</td>
                        <td>
                            <a  href="{{ route('email.delete-inbox',['id'=>$item->id]) }}"  title="@lang('site.delete')" onclick="return confirm('@lang('admin.delete-prompt')')" ><i class="fa fa-trash"></i></a>
                        </td>
                    </tr>
                    @endforeach
                    </tbody></table>
